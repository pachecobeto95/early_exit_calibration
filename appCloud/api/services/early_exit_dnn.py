...
class Early_Exit_DNN_CALTECH(nn.Module):
  ...
  def forwardAllSamplesCalibCloudInference(self, x, conf_list, class_list, p_tar, nr_branch_edge):
    ...
    output = self.classifier(x)
    output = self.temperature_scale_branches(output, self.temperature_all_samples, -1)
...
class Early_Exit_DNN_CIFAR(nn.Module):
  ...
  def select_dnn_architecture_model(self):
    ...
    architecture_dnn_model_dict = {"mobilenet": ee_mobilenet,
                                   "resnet18": self.early_exit_resnet18,
                                   "vgg16": self.early_exit_vgg16, 
                                   "resnet152": self.early_exit_resnet152}
  ...
  def early_exit_vgg16(self):
    ...
    if (self.pretrained):
      ...
      backbone_model = vgg16_bn(self.n_classes)
  ...
  def forwardCloudNoCalibInference(self, x, p_tar, nr_branch_edge):
  ...
  def forwardCloudOverallCalibInference(self, x, p_tar, nr_branch_edge):


    for i, exitBlock in enumerate(self.exits[int(nr_branch_edge):], nr_branch_edge):
        x = self.stages[i](x)
  ...
  def forwardCloudBranchesCalibInference(self, x, p_tar, nr_branch_edge):
