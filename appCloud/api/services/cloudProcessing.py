...
def cloudNoCalibInference(feature, conf_list, class_list, p_tar, nr_branch_edge):
...
def cloudOverallCalibInference(feature, conf_list, class_list, p_tar, nr_branch_edge):
	feature = torch.Tensor(feature).to(config.device).float()
	conf, infer_class = ee_dnn_overall_calib_inference(feature, conf_list, class_list, p_tar, nr_branch_edge)
	return {"status": "ok"}

def cloudBranchesCalibInference(feature, conf_list, class_list, p_tar, nr_branch_edge):
	feature = torch.Tensor(feature).to(config.device).float()
