	...
	for i, (data, target) in enumerate(test_loader, 1):
		...
		for nr_branch_edge in nr_branch_edge_list:
			...
			for p_tar in p_tar_list:
				...
				sendImage(filepath, i, config.url_edge_no_calib_only_edge_standard_dnn, target, p_tar, nr_branch_edge)
				sendImage(filepath, i, config.url_edge_overall_calib_only_edge_standard_dnn, target, p_tar, nr_branch_edge)
				sendImage(filepath, i, config.url_edge_branches_calib_only_edge_standard_dnn, target, p_tar, nr_branch_edge)
...
def main(args):
	...
	logPath = "./logTest_Edge_Only_Standard_Dnn_%s_%s.txt"%(args.model_name, args.dataset_name)
