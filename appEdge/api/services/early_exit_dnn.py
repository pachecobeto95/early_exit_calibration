...
class Early_Exit_DNN(nn.Module):
  ...
  def forwardEdgeAllSamplesCalibInference(self, x, p_tar, nr_branch_edge):
    conf_list, class_list = [], []
    n_exits = self.n_branches + 1
    ...
    for i, exitBlock in enumerate(self.exits[:int(nr_branch_edge)]):
      ...
      output_branch = exitBlock(x)
      output_branch = self.temperature_scale_branches(output_branch, self.temperature_all_samples, i)
